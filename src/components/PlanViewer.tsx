...
function DifficultyBadge({ level }: { level: string }) {
  ...
  return (
    <span
      className={`px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wide ${colors[level] || "bg-gray-100 text-gray-700"}`}
    >
      {level}
    </span>
  );
...
export default function PlanViewer({ plan, onUpgradeClick }: { plan: Plan; onUpgradeClick?: (upgrade: string) => void }) {
  ...
  return (
    <div>
      ...
      {/* Platform badge */}
      <div className="bg-brand-accent-light border border-amber-200/40 rounded-lg px-4 py-3 mb-6 flex items-start gap-3">
        <span className="font-mono text-xs font-medium text-amber-700 bg-amber-200/30 px-2 py-0.5 rounded shrink-0">
          {plan.platform.name}
        </span>
      ...
      {!plan.in_scope && plan.scope_note && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 mb-6">
          ...
          <p className="text-sm text-blue-800">{plan.scope_note}</p>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-px bg-gray-100 rounded-lg overflow-hidden">
            {[
              ...
              { label: "Wiring", value: `${plan.wiring.length} conn.` },
              { label: "Code", value: `${plan.full_code.code.split("\n").length} lines` },
            ].map((s, i) => (
              <div key={i} className="bg-brand-bg p-4 text-center">
                <div className="text-xl font-bold">{s.value}</div>
                <div className="text-xs text-brand-secondary mt-0.5">{s.label}</div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white border border-gray-100 rounded-lg p-4">
            <h3 className="text-xs font-semibold text-brand-secondary uppercase tracking-wide mb-3">
              Libraries
            </h3>
            {plan.libraries.map((lib, i) => (
              <div
                key={i}
                className={`flex justify-between items-baseline py-1.5 ${i ? "border-t border-gray-50" : ""}`}
              >
                <span className="font-mono text-sm font-medium">{lib.name}</span>
                <span className="text-xs text-brand-tertiary max-w-[55%] text-right">
                  {lib.purpose}
                </span>
              </div>
            ))}
          ...
          {/* Power notes */}
          <div className="bg-white border border-gray-100 rounded-lg p-4">
            <h3 className="text-xs font-semibold text-brand-secondary uppercase tracking-wide mb-2">
              Power
            </h3>
          ...
          {plan.warnings.length > 0 && (
            <div className="bg-red-50/50 border border-red-100 rounded-lg p-4">
              ...
              {plan.warnings.map((w, i) => (
                <p key={i} className="text-xs text-red-800 leading-relaxed pl-3 relative mb-1">
          ...
          <div className="bg-white border border-gray-100 rounded-lg p-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {plan.next_upgrades.map((u, i) => (
                <button
                  ...
                  className="text-xs text-brand-secondary bg-gray-50 hover:bg-amber-50 hover:text-brand-accent px-2.5 py-1 rounded-full transition-colors cursor-pointer text-left"
            ...
            {onUpgradeClick && (
              <p className="text-[11px] text-brand-tertiary mt-2">Click any upgrade to generate a new plan with it included</p>
            )}
          </div>
        </div>
      )}
      ...
      {activeTab === "components" && (
        <div className="bg-white border border-gray-100 rounded-lg overflow-hidden">
          {/* Header */}
          <div className="grid grid-cols-[1fr_auto_auto] px-4 py-2.5 border-b border-gray-50 text-[11px] text-brand-tertiary font-semibold uppercase tracking-wider">
            <span>Component</span>
          ...
          {plan.components.map((c, i) => (
            <div
              key={i}
              className={`grid grid-cols-[1fr_auto_auto] px-4 py-3 items-start ${i ? "border-t border-gray-50" : ""}`}
            >
              <div>
                <div className="text-sm font-medium">{c.name}</div>
                <div className="text-xs text-brand-tertiary mt-0.5">{c.notes}</div>
              </div>
              <span className="text-sm text-brand-secondary text-right pl-4">
                ×{c.quantity}
              </span>
          ...
          {/* Total */}
          <div className="grid grid-cols-[1fr_auto] px-4 py-3.5 border-t-2 border-gray-100 bg-amber-50/50">
      ...
      {activeTab === "wiring" && (
        <div className="bg-white border border-gray-100 rounded-lg overflow-hidden">
          {plan.wiring.map((w, i) => (
            <div
              key={i}
              className={`px-4 py-3 ${i ? "border-t border-gray-50" : ""}`}
            >
              {/* Connection: FROM → TO */}
              <div className="flex items-center gap-2 flex-wrap">
                <span className="font-mono text-xs font-medium text-brand-accent bg-amber-50 px-2 py-1 rounded">
                  {w.from}
                </span>
                ...
                <span className="font-mono text-xs font-medium bg-gray-50 px-2 py-1 rounded">
                  {w.to}
                </span>
              </div>
              ...
              {w.notes && (
                <p className="text-xs text-brand-tertiary mt-1.5 leading-relaxed">
                  {w.notes}
                </p>
              )}
            </div>
